...
void handle_signals(int signo)
{
	fprintf(stderr, "[DEBUG] handle_signals called with signo=%d\n", signo);
	if (g_child_pid != 0) // Si un processus fils est actif
	{
		kill(g_child_pid, signo); // Transmettre le signal au fils
		return;
	}
...
t_token *tokenize_input(const char *input, t_shell *shell)
{
	...
	// Vérification globale des quotes
	...
	while (input[i])
	{
		...
		if ((input[i] == '&' && input[i + 1] == '&') || 
			(input[i] == '|' && input[i + 1] == '|'))
		{
			token_add(&tokens, token_new(ft_substr(input, i, 2), TOKEN_UNSUPPORTED, NO_QUOTE));
			i += 2;
			continue ;
		}
		...
		if (input[i] == '<' || input[i] == '>')
		{
			if (input[i + 1] == input[i]) // Cas heredoc `<<` ou append `>>`
			{
				...
				if (type == TOKEN_HEREDOC) // Si heredoc, lire le délimiteur brut
				{
					i = get_next_index(input, i);
					start = i;
					if (input[i] == '\'' || input[i] == '"')
					...
					token_add(&tokens, token_new(ft_substr(input, start, i - start), TOKEN_WORD, NO_QUOTE));
				}
				continue ;
			}
			else // Cas simple `<` ou `>`
			{
				...
				subtype = (input[i] == '<') ? TOKEN_REDIRECT_IN : TOKEN_REDIRECT_OUT;
				token_add(&tokens, token_new(ft_substr(input, i, 1), subtype, NO_QUOTE));
				i++;
				continue ;
			}
		}
...
int	builtin_cd(t_command *cmd, t_shell *shell)
{
	...
	else if (ft_strcmp(cmd->argv[1], ".") == 0)
	{
		...
		path = ft_strdup(cwd); // Le chemin reste le même
	}
	...
	while (cmd)
	{
		...
		else
		{
			...
			if (pid == 0)
			{
				...
				{
					char *path = resolve_command_path(cmd->argv[0], shell->env);
					if (!path)
					{
...
int	builtin_exit(char **args, t_shell *shell)
{
	if (args[1])
	{
		...
		shell->exit_stat = ft_atoi(args[1]);
	}
	exit(shell->exit_stat);
}
/*
...
int	builtin_export(char **args, t_env_var **env, t_shell *shell)
{
	...
	if (equal_pos)
		...
		name = ft_strdup(args[1]); // Aucun '=' => variable seule
	...
	{
		value = ft_strdup(equal_pos + 1);
		if (!name || !value) // Gestion des erreurs mémoire
			return (free(name), free(value), 1);
...
int	validate_commands(t_command *commands)
{
	...
	while (cmd)
	{
		...
		{
			ft_putstr_fd("minishell: syntax error: unexpected empty command\n", STDERR_FILENO);
			return (-1);
		}
